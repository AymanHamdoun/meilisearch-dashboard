...
import { createNavItem, createDropdownItem, NavItem } from '../../types/navigation';
...
const SideBar = () => {
    ...
    const sideBarLinks = useMemo(() => {
        ...
        if (featureChildren.length > 0) {
            baseLinks.splice(2, 0, createDropdownItem("s-features", "Features", FeaturesIcon, featureChildren));
        }
    ...
    return (
        <div>
            <aside id="default-sidebar"
                className="fixed top-0 left-0 z-40 w-64 pt-5 h-screen transition-transform -translate-x-full sm:translate-x-0 border-r border-r-gray-200"
                aria-label="Sidebar">
                <div className="h-full px-0 pb-4 overflow-y-auto bg-white">
                    ...
                    <ul className="space-y-2 font-medium">
                        {sideBarLinks.map((sideBarLink, i) => (
                            <DynamicSideBarLink key={i} sideBarLink={sideBarLink} />
                        ))}
                    </ul>
                </div>
            </aside>
...
const DynamicSideBarLink = ({ sideBarLink }: { sideBarLink: NavItem }) => {
